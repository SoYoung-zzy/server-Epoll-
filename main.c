...
static int server_runing_flag = 1; // whether exit accept is or not
static int exit_flag = 0; // whether exit epoll wait is or not
...
static void connect_total_count_opration(BOOL add_or_subract, int value)//增删连接总数（互斥操作）
{
...
static void signal_handler_reboot(int32_t theSignal)
{
	...
	signal(SIGPIPE, SIG_IGN);//忽略掉SIGPIPE 以免进程被终止掉
	if (SIGKILL == theSignal || SIGTERM == theSignal) //SIGKILL不能被忽略
	{
...
/****************************** accept task ******************************************/
static void *accept_thread(void *arg)//I/O线程
{
	int connect_fd = -1;
	socklen_t clilen = 0;
	...
	if (-1 == sockfd) // INVALID_SOCKET
	{
		LOG_INFO(LOG_LEVEL_ERROR, "create socket error.\n");
		return NULL;
	}
	//优化 set socket options 服务器快速重启 
	...
	serveraddr.sin_port = htons(port); //serveraddr.sin_port  = htons(SERVER_LISTEN_PORT);
	...
	listen(sockfd, LISTENQ); // 4096
	clilen = sizeof(cliaddr);
	while (server_runing_flag=1)//while(server runing)
	{
		//accept此连接
		if (current_connected_total < MAX_EVENTS) // effective
		{
			if ((connect_fd = accept(sockfd, (struct sockaddr *)&clientaddr, &clilen)) < 0)
			{
		...
		{
			sleep(1);
			snprintf(log_str_buf, LOG_STR_BUF_LEN, "current accept number have up to maximum(%d) .\n", MAX_EVENTS);
		...
		if (epoll_connect_event_index == -1) 
		{
			...
			if (connect_fd != -1)
			{
				closesocket(connect_fd);//端口复用 为下一个监听服务器
				connect_fd = -1;
			}
			continue;
		}
		//设置socket连接为非阻塞模式 set connect fd non blocking
		...
		// 为此连接设置event(EPOLLIN | EPOLLET ...);并注册事件
		...
		connect_total_count_opration(TRUE, 1);//作totalnum+1
...
int main(int argc, char *argv[])
{
	...
	signal(SIGCHLD, SIG_IGN); // Ignore the child to the end of the signal, preventing the zombie process
	// init log
	...
	//creat a new  session for child process
	setsid();
...
//	g_net_init_mysql(); // connect to the database
	...
	pthread_create(&accep_thread_t, NULL, accept_thread, NULL);

	time(&prevTime);
	eventTime = prevTime;
	while (!exit_flag)//无限循环while (1)
	{
		time(&now);
		...
		for (index = 0; index < epoll_events_number; ++index) // deal with the event
		{
			...
			// delete epoll event epoll_wait 函数相当于把客户端的连接从epoll中拿出来
			...
			if (events[index].events & EPOLLIN) //have read event
			{
				...
				if (0 == recv_buffer_from_fd(connect_socket_fd_temp, recv_buffer, &recv_length))//recv  success
				{
					...
					// 接受了一个完整请求然后交给工作线程（线程池）处理receive buffer success then add the thread pool
