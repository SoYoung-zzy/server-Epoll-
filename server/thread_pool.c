 ...
 * thread_numbers: the thread pool number(the number of concurrent threads)
 */
thpool_t *thpool_init(int thread_numbers)
{
	int index = 0;
    thpool_t *thpool = NULL;

    if(!thread_numbers || thread_numbers < 1)
    {
        thread_numbers = 1;
    }
    ...
    thpool->threadsN = thread_numbers;
    thpool->threads = (pthread_t*) malloc(thread_numbers * sizeof(pthread_t));
    ...
    for(index = 0; index < thread_numbers; ++index)
    {
...
{
	//thpool_t *tp_p=(thpool_thread_parameter*)thread_paramter;
	thpool_t *tp_p = thread_paramter->thpool;
	int index = thread_paramter->thread_index;
	printf("index = %d\n", index);
    while(thpool_keepalive ==1)
    {
        if(sem_wait(tp_p->jobqueue->queueSem)) //P(P/V)  thread block, until have data come
        {
            perror("thpool_thread_do(): Waiting for semaphore");
            exit(1);
        }
        //get a job for this thread
		if(thpool_keepalive)
        {
            FUNC function;
//            void *arg_buff;
            thpool_job_t *job_p;
            pthread_mutex_lock(&mutex);//访问工作队列要加锁
            job_p = thpool_get_jobqueue_tail(tp_p);
...
//get the queue tail
static thpool_job_t* thpool_get_jobqueue_tail(thpool_t* tp_p)
{
...
static void thpool_jobqueue_add(thpool_t *tp_p, thpool_job_t *newjob_p)
{
    ...
    (tp_p->jobqueue->jobN)++;
    sem_post(tp_p->jobqueue->queueSem);//V（P/V）
    sem_getvalue(tp_p->jobqueue->queueSem, &reval);
