...
static int create_connect(POOL_SQL_SOCK *pool_sql_sock, SQL_SOCK_NODE *node)
{
	...
	do
	{
		if (NULL == mysql_init(&node->fd))// MySQL *mysql_init(MYSQL *mysql);
		{
		...
		else//初始化MYSQL对象成功
		{
			...
			{
				node->used = 0;//未使用
				node->sql_state = DB_CONN;//已连接数据库
				mysql_options(&node->fd, MYSQL_OPT_RECONNECT, &opt);//设置自动连接开启
...
int sql_pool_create(int connect_pool_number)
{
	...
	//strcpy(sql_sock_pool->db_name, db_name);     //数据库名字
   //strcpy(sql_sock_pool->user, user);           //使用用户名
   //strcpy(sql_sock_pool->passwd, passwd);       //密码
    mysql_init(&fd_temp);//？？？
...
{
	int index;
	for (index = 0; index < sql_sock_pool.pool_number; ++index)//关闭连接池中所有连接
	{
		if (NULL != sql_sock_pool.sql_pool[index].mysql_sock) // close the mysql    MYSQL* mysql_sock
		{
...
//从连接池中取出一个能够使用的连接；为了保证取出每一个连接的可能性大致相同，
//这里将开始访问地址下标用随机数；在取出时也做了连接状态检测，若断开则重新建立连接
SQL_SOCK_NODE *get_db_connect_from_pool()
{
	...
	start_index = rand() % sql_sock_pool.pool_number;//0 ~ sql_sock_pool.pool_number
